...
public class WorkoutTypeControllerTest {
    ...
    void getWorkoutTypes_shouldReturnList() throws Exception {
        ...
        WorkoutTypeDTO mockType = new WorkoutTypeDTO(1L, "臥推", "胸", List.of());
        ...
        mockMvc.perform(get("/workout-types"))
                ...
                .andExpect(jsonPath("$[0].mainTag").value("胸")); // 預期回傳 muscleGroup
    }

    @Test
    void createWorkoutType_shouldSucceed() throws Exception {
        // Arrange
        CreateTypeRequestDTO requestDTO = new CreateTypeRequestDTO("引體向上", "背", List.of());
        WorkoutType mockResponse = new WorkoutType("引體向上", "背",  List.of(), false);
        mockResponse.setId(1L);

        when(workoutTypeService.createWorkoutType(any(CreateTypeRequestDTO.class)))
        ...
        mockMvc.perform(post("/workout-types")
                ...
                .andExpect(jsonPath("$.mainTag").value("背"))
                .andExpect(jsonPath("$.name").value("引體向上"))
                .andExpect(jsonPath("$.secondaryTags.length()").value(0));
    }
    ...
    void deleteWorkoutTypeTestReverse() throws Exception{
        ...
        // 模擬資料庫查不到資料
        doThrow(new ApiException(ApiErrorCode.TYPE_NOT_FOUND, null, HttpStatus.NOT_FOUND)).when(workoutTypeService).deleteCustomedWorkoutType(workoutTypeId);
        //Act & Assert
